...
model = model_builder.build_NeoConv_Trans(num_heads = 1, dff = 32, numClass = numClass, d_model = 32, dropoutrate = 0.2, conv_filters = 5, conv_kernel = 3)
...
def build_NeoConv_Trans(num_heads = 8, dff = 64, numClass = 47, d_model = 64,
    ...
    #m_input = tf.random.uniform((4, 32, 16, 8,1), dtype=tf.float32, minval=0, maxval=200)
    m_input = keras.Input(shape = (128, 64, 50, 1))
    m_output = layers.Conv3D( filters = 10, kernel_size = conv_kernel, padding='same', activation='relu', input_shape=(128, 64 ,50, 1))(m_input)        
    m_output = layers.BatchNormalization()(m_output)
    m_output = layers.AveragePooling3D(pool_size=(2, 2, 3))(m_output)
    m_output = layers.Dropout(dropoutrate)(m_output)
    m_output = layers.Conv3D( filters = 20, kernel_size = conv_kernel, padding='same', activation='relu')(m_output)        
    m_output = layers.BatchNormalization()(m_output)
    m_output = layers.AveragePooling3D(pool_size=(2, 2, 3))(m_output)
    m_output = layers.Dropout(dropoutrate)(m_output)
    m_output = layers.Conv3D( filters = 40, kernel_size = conv_kernel, padding='same', activation='relu')(m_output)        
    m_output = layers.BatchNormalization()(m_output)
    m_output = layers.AveragePooling3D(pool_size=(2, 2, 1))(m_output)
    m_output = layers.Dropout(dropoutrate)(m_output)
    m_output = layers.Conv3D( filters = 80, kernel_size = conv_kernel, padding='same', activation='relu')(m_output)        
    m_output = layers.BatchNormalization()(m_output)
    m_output = layers.AveragePooling3D(pool_size=(2, 2, 1))(m_output)
    m_output = layers.Dropout(dropoutrate)(m_output)
    m_output = layers.Conv3D( filters = 5, kernel_size = (1,1,1), padding='same', activation='relu')(m_output)      
    m_output = layers.BatchNormalization()(m_output)
    m_output = layers.Dropout(dropoutrate)(m_output)
    print(m_output.shape[1:].as_list())       
    pos_dim = m_output.shape[1:].as_list()
